...
void calbias(int** Lintab,int* sblks_to_consider,double totalbias,int noofvars,int noofrounds,int** Path);
void dosubtitute(int** Lintab,int* sblks_to_consider,int indextosubtitute,double totalbias,int noofvars,int noofrounds,int** Path);
double max_bias;
int** MaxPath;
int globalcount;
int main(){
	FILE *ptr=fopen("Linearapproxtable2.txt","r");
	int i,j,k;
		...
		for(j=0;j<16;j++){
			...
			//	for(m=0;m<16;m++){
					if(j!=0){
						...
						//sblks_to_consider[(i+4)%8]=l;
						//sblks_to_consider[(i+7)%8]=m;
						noofvars=0;
						noofrounds=0;
						for(k=0;k<8;k++){
						...
						totalbias=1.0;
						calbias(Lintab,sblks_to_consider,totalbias,noofvars,noofrounds,Path);
						if(sblks_to_consider[0]==1 && sblks_to_consider[4]==0 && sblks_to_consider[7] ==0  && sblks_to_consider[2] ==0 &&
							sblks_to_consider[3] ==0 && sblks_to_consider[5] ==0 && sblks_to_consider[6] ==0 &&sblks_to_consider[1] ==0){
							printf("coming here");	
						}
			//		}
			//	}
	...
	printf("Final max bias:%.20f  %d\n",max_bias,globalcount);
	for(i=0;i<32;i++){
		for(j=0;j<8;j++){
...
///Calculating bias ,recursive function
void calbias(int** Lintab,int* sblks_to_consider,double totalbias,int noofvars,int noofrounds,int** Path){
	if(noofrounds==6){
		globalcount++;
		double temp=totalbias*powers[noofvars-1];
		int i,j;
		//some debug print satements
		if(globalcount%2==0){
			if(totalbias==(double)0){printf("How is it happening!!!"); }
			printf("global count:	%d  %.20f %d\n",globalcount,max_bias ,noofvars);
			
		}
		if(fabs(temp) > fabs(max_bias)){
			//printf("%lf %d -what\n",totalbias,globalcount);
			for(i=0;i<32;i++){
				for(j=0;j<8;j++){
		...
		dosubtitute(Lintab,sblks_to_consider,0,totalbias,noofvars,noofrounds+1,Path);
	}
}
...
void dosubtitute(int** Lintab,int* sblks_to_consider,int indextosubtitute,double totalbias,int noofvars,int noofrounds,int** Path){
	if(indextosubtitute==8){
		...
		free(output1);
		free(output2);
		//printf("%lf %d\n",totalbias,noofrounds);
		//For next round
		//Path[2*noofrounds]=(int *)memcpy((void*)(Path[2*noofrounds]),(void*)sblks_to_consider,8*sizeof(int));
		for(i=0;i<8;i++){
		...
		for(i=0;i<8;i++){
			if(i<4){
				temp[i]=sblks_to_consider[i];
				sblks_to_consider[i]=sblks_to_consider[i+4];
			}
			else{
				sblks_to_consider[i]=temp[i-4];
			}
		}
		*/
		for(i=0;i<8;i++){
		...
		if(sblks_to_consider[indextosubtitute]!=0){
			...
			for(a1=0;a1<16;a1++){
				double temp3;
				double temp2=(double)(((double)Lintab[sblks_to_consider[indextosubtitute]][a1]-(double)8)/(double)16);
				if(temp2>=0.25 || temp2<=-0.25 ){
					
					sblks_to_consider[indextosubtitute]=a1;
					if(totalbias==(double)0){printf("How is it happening!!!"); }
					temp2=totalbias*temp2;
					temp3=temp2*powers[noofvars];
					int i,success=0;
					if(fabs(temp3)<fabs(max_bias) ){
						...
						if(i==8){
						//	printf("crossing for best one %.20f %.20f %d\n",temp3,max_bias,noofvars);
						}
			...
			dosubtitute(Lintab,sblks_to_consider,indextosubtitute+1,totalbias,noofvars,noofrounds,Path);
		}
		return;
	} 	
